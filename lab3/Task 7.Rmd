  ...
  selectInput("plotg", "Choose plot type",
      list(`points` = c("g", "gheat", "gratio"),
           `point size` = c("gsheat", "gsratio"),
            `Linear Regression` = c("g_lm"), 
         `Points with Line Segments` = c("g_lines"))
      )
    )
...
renderPlot({
 ...
 g = ggplot(fin.df, aes(x = RATIO, y = HEAT)) + geom_point() 
  ...
  g_lm <- g + geom_smooth(method = "lm", color = "blue")

  g_lines <- g + geom_line(aes(group = 1), color = "black")
 ...
 if (input$plotg == "g_lm") print(g_lm)

 if (input$plotg == "g_lines") print(g_lines)
